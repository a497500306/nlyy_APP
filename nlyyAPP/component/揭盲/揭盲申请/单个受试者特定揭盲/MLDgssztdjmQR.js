...
var DgssztdjmQR = React.createClass({
    getDefaultProps(){
        return {
            //用户信息
            users:null,
    ...
    renderRow(rowData){
        ...
        if(rowData == "申请类型") {
            return(
                <MLTableCell title={rowData} rightTitle={this.props.UnblindingType == 1 ? '单个受试者特定揭盲' : '单个受试者紧急揭盲'} isArrow={false}/>
            )
        }
        if(rowData == "受试者编号") {
            return(
    ...
    getLogin(){
        ...
        //获取中心数据网络请求
        fetch(settings.fwqUrl + "/app/getUnblindingApplication", {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                StudySeq:this.props.users.StudySeq,
                SiteID:this.props.users.SiteID,
                SiteNam:this.props.users.SiteNam,
