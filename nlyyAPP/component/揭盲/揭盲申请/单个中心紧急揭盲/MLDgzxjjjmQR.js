...
var DgzxjjjmQR = React.createClass({
    getDefaultProps(){
        return {
            //用户信息
            core:null,
    ...
    renderRow(rowData){
        ...
        if(rowData == "申请类型") {
            return(
                <MLTableCell title={rowData} rightTitle={'单个中心紧急揭盲'} isArrow={false}/>
            )
        }
        if(rowData == "中心编号") {
            return(
                <MLTableCell title={rowData} rightTitle={this.props.core.SiteID} isArrow={false}/>
            )
        }
        if(rowData == "中心名称") {
            return(
                <MLTableCell title={rowData} rightTitle={this.props.core.SiteNam} isArrow={false}/>
            )
        }
        if (rowData == "揭盲原因") {
            return(
    ...
    getLogin(){
        ...
        //获取中心数据网络请求
        fetch(settings.fwqUrl + "/app/getSiteUnblindingApplication", {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                StudyID:this.props.core.StudyID,
                StudySeq:Users.Users[0].StudySeq,
                SiteID:this.props.core.SiteID,
                SiteNam:this.props.core.SiteNam,
                site:this.props.core,
                UnblindingType:3,
