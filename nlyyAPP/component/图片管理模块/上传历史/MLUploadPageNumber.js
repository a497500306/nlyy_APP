...
var MLUploadPageNumberLish = require('./MLUploadPageNumberLish')

var MLUploadPageNumber = React.createClass({
    //耗时操作,网络请求
    componentDidMount(){
        var UserSite = '';
        var UserSiteYN = '';
        ...
        fetch(settings.fwqUrl + "/app/getImagePageNumberListStatistics", {
            method: 'POST',
            headers: {
            ...
            .then((responseJson) => {
                ...
                if (responseJson.isSucceed != 400){
                    ...
                    var tableData = [];
                    for (var i = 0 ; i< researchParameter.researchParameter.CRFMaxNum ; i++){
                        tableData.push("页码" + (i + 1));
                    }
                    //ListView设置
        ...
        if (this.state.animating == true) {
            return (
                <View style={styles.container}>
                    <MLNavigatorBar title={'页码上传历史'} isBack={true} backFunc={() => {
        ...
        return (
            <TouchableOpacity onPress={()=> {
                ...
                this.props.navigator.push({
                    component: MLUploadPageNumberLish, // 具体路由的版块
                    //传递参数
                    passProps:{
                ...
                <MLTableCell title={rowData} rightTitle={this.state.data[rowID] + "人"}/>
            </TouchableOpacity>
        )
    }
})
...
// 输出组件类
module.exports = MLUploadPageNumber;